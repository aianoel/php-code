...
$stmt = $pdo->prepare("
    SELECT 
        ...
        SUM(COALESCE(s.units, 1)) as total_units,
...
if (!empty($teachers)) {
    foreach ($teachers as $teacher) {
        $stmt = $pdo->prepare("
            ...
            GROUP BY 
                c.id, c.section, c.schedule, c.room, c.capacity, c.school_year, c.semester,
                s.name, s.code, s.units, s.department
            ORDER BY 
                s.department, s.name
        ");
...
<head>
    ...
    <title>Teacher Load - Academic Coordinator</title>
